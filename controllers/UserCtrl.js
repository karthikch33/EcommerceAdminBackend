...
export const makeAdmin = asyncHandler(async (req,res)=>{
    ...
    try {
        ...
        if(updateUser){ res.json({status:201,message:"User Status Updated"})}

        res.json({status:500,message:"User Status Not Updated"})
...
export const loginAdmin = asyncHandler(async (req,res)=>{
    ...
    if(!findAdmin) res.json({statusSent:404,message:'Admin Data Not Recorded'})
    if(findAdmin.role !== 'admin')  res.json({statusSent:209,message:'You are not an admin'})
    ...
    if(findAdmin && await findAdmin.isPasswordMatched(password)){
        ...
        res.json({
            ...
            admin:findAdmin?.role,
            statusSent:201,
            message:"Login Success",
            refreshToken:generateToken(findAdmin?._id)
        })
    }else{
        res.json({statusSent:404,message:"Login Failed"})
    }
})
...
export const activeStatus = asyncHandler(async (req,res)=>{
    ...
    try {
        const user = await User.findById(_id)
        const user2 = await User.findByIdAndUpdate(_id,{
            isBlocked: !user?.isBlocked
        },{new:true})
        user !== undefined ? res.json({status:201,message:"User Updated"}).status(201) : res.json({status:404,message:"User Not Found"}).status(404)
    } catch (error) {
        console.log(error)
    }
})
